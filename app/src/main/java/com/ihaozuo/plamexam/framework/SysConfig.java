package com.ihaozuo.plamexam.framework;

/**
 * Created by zhangzhongyao on 16/10/09.
 */
public class SysConfig {
    public static final String CURRENT_BASE_VERSION = "V2";
//    public static final String[] BASE_API = new String[]{"http://hc.ihaozhuo.com:90", "HZ_PME_API_V1", "1!2@3#4$5%6^"};//pro
//    public static final String[] BASE_API = new String[]{"http://hz3bn04d2/HZ.WebAPI/", "HZ_PME_API_V1", "1!2@3#4$5%6^"};//xw
    public static final String[] BASE_API = new String[]{"http://hz3bn04d2:8070/", "HZ_PME_API_V1", "1!2@3#4$5%6^"};//Test

    public static final String CONTROLLER_PRE_API_VALUES = "api/" + CURRENT_BASE_VERSION + "/Values/" ;

//    public static final String CONTROLLER_PRE_API_VALUES = "api/" + CURRENT_BASE_VERSION + "/" ;

    public static final int CONNECT_TIMEOUT = 15;
    public static final int WRITE_TIMEOUT = 15;
    public static final int READ_TIMEOUT = 40;

}
